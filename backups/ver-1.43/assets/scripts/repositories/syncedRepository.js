(function () {
  ...
  function normalizeDerivedRefreshTargets(value) {
    const input = value || {};

    return {
      animalIds: Array.from(new Set((input.animalIds || []).filter(Boolean).map((animalId) => String(animalId)))),
      sessionIds: Array.from(new Set((input.sessionIds || []).filter(Boolean).map((sessionId) => String(sessionId)))),
    };
  }
  ...
  function normalizeSyncMeta(value, defaultClinicId) {
    ...
    return {
      ...
      syncing: false,
      pendingDerivedRefresh: normalizeDerivedRefreshTargets(input.pendingDerivedRefresh || base.pendingDerivedRefresh),
    };
  }
  ...
  function createDerivedRefreshTargets() {
    return {
      animalIds: [],
      sessionIds: [],
    };
  ...
  function addUniqueTarget(targets, key, value) {
    if (!targets || !value || !Array.isArray(targets[key]) || targets[key].indexOf(value) >= 0) {
      return;
    }
  ...
  function addDerivedRefreshForVisit(targets, visitRecord, previousVisitRecord) {
    if (!targets || !visitRecord) {
      return;
    }

    addUniqueTarget(targets, "animalIds", visitRecord.animalId);
    addUniqueTarget(targets, "sessionIds", getRecordSessionId(visitRecord));

    if (previousVisitRecord) {
      addUniqueTarget(targets, "sessionIds", getRecordSessionId(previousVisitRecord));
    }
  }

  function mergeDerivedRefreshTargets(targets, nextTargets) {
    const normalizedTargets = normalizeDerivedRefreshTargets(nextTargets);

    normalizedTargets.animalIds.forEach((animalId) => {
      addUniqueTarget(targets, "animalIds", animalId);
    });
    normalizedTargets.sessionIds.forEach((sessionId) => {
      addUniqueTarget(targets, "sessionIds", sessionId);
    });
  }

  function hasDerivedRefreshTargets(targets) {
    const normalizedTargets = normalizeDerivedRefreshTargets(targets);
    return normalizedTargets.animalIds.length > 0 || normalizedTargets.sessionIds.length > 0;
  }

  function countDerivedRefreshTargets(targets) {
    const normalizedTargets = normalizeDerivedRefreshTargets(targets);
    return normalizedTargets.animalIds.length + normalizedTargets.sessionIds.length;
  }
  ...
  SyncedEmbryoRepository.prototype.bootstrap = async function (settings) {
    ...
    if (this.remote && this.syncMeta.networkOnline) {
      ...
      if (hasLocalCache) {
        scheduleBootstrapSync = this.hasQueuedLocalWork() || !this.syncMeta.lastSyncAt;
      } else {
  ...
  SyncedEmbryoRepository.prototype.getSyncState = function () {
    const pendingCount = this.syncMeta.queue.length + countDerivedRefreshTargets(this.syncMeta.pendingDerivedRefresh);
    let state = "synced";
  ...
  SyncedEmbryoRepository.prototype.hasQueuedLocalWork = function () {
    return this.syncMeta.queue.length > 0 || hasDerivedRefreshTargets(this.syncMeta.pendingDerivedRefresh);
  };
  ...
  SyncedEmbryoRepository.prototype.scheduleQueuedLocalChangesFlush = function (options) {
    ...
    if (!this.remote || !this.syncMeta.networkOnline || !this.hasQueuedLocalWork()) {
      return;
    }
  ...
  SyncedEmbryoRepository.prototype.addPendingDerivedRefresh = function (targets) {
    const currentTargets = normalizeDerivedRefreshTargets(this.syncMeta.pendingDerivedRefresh);

    mergeDerivedRefreshTargets(currentTargets, targets);
    this.syncMeta.pendingDerivedRefresh = currentTargets;
  };

  SyncedEmbryoRepository.prototype.removePendingDerivedRefresh = function (targets) {
    const normalizedTargets = normalizeDerivedRefreshTargets(targets);
    const currentTargets = normalizeDerivedRefreshTargets(this.syncMeta.pendingDerivedRefresh);

    this.syncMeta.pendingDerivedRefresh = {
      animalIds: currentTargets.animalIds.filter((animalId) => normalizedTargets.animalIds.indexOf(animalId) < 0),
      sessionIds: currentTargets.sessionIds.filter((sessionId) => normalizedTargets.sessionIds.indexOf(sessionId) < 0),
    };
  };

  SyncedEmbryoRepository.prototype.flushPendingDerivedRefresh = async function () {
    const targets = normalizeDerivedRefreshTargets(this.syncMeta.pendingDerivedRefresh);

    if (!hasDerivedRefreshTargets(targets)) {
      return false;
    }
    ...
    if (typeof this.remote.recomputeAnimalRollup !== "function" || typeof this.remote.recomputeSessionsDateRange !== "function") {
      this.removePendingDerivedRefresh(targets);
      return false;
    }

    for (let index = 0; index < targets.animalIds.length; index += 1) {
      await this.remote.recomputeAnimalRollup(this.defaultClinicId, targets.animalIds[index]);
    }

    await this.remote.recomputeSessionsDateRange(this.defaultClinicId, targets.sessionIds);
    this.removePendingDerivedRefresh(targets);
    return true;
  };
  ...
  SyncedEmbryoRepository.prototype.pushQueuedUpsert = async function (entry, localRecord, remoteRecord, remoteTombstone) {
    ...
    if (remoteRecord && recordsMatchAfterSync(localRecord, remoteRecord)) {
      const derivedRefresh = createDerivedRefreshTargets();

      if (entry.entityType === "visit" && typeof this.remote.saveVisitRecord === "function") {
        addDerivedRefreshForVisit(derivedRefresh, localRecord, remoteRecord);
      }
    ...
    if (entry.entityType === "session") {
      ...
      upsertResult = await this.upsertRemoteVisit(localRecord, {
    ...
    return {
      remoteChanged: true,
      derivedRefresh: upsertResult && upsertResult.derivedRefresh,
    };
  };
    ...
    this.localFlushPromise = (async () => {
      ...
      try {
        ...
        for (let index = 0; index < entries.length; index += 1) {
          ...
          mergeDerivedRefreshTargets(derivedRefresh, result && result.derivedRefresh);
          this.emitSyncState();
        }

        if (!needsFullSync && hasDerivedRefreshTargets(derivedRefresh)) {
          this.addPendingDerivedRefresh(derivedRefresh);
          await this.persistState();
        }

        if (!needsFullSync && hasDerivedRefreshTargets(this.syncMeta.pendingDerivedRefresh)) {
          const derivedChanged = await this.flushPendingDerivedRefresh();
          remoteChanged = remoteChanged || derivedChanged;
          this.emitSyncState();
        }
        ...
        } else if (this.hasQueuedLocalWork()) {
  ...
  SyncedEmbryoRepository.prototype.upsertRemoteVisit = async function (visitRecord, options) {
    const settings = options || {};
    const visitPayload = getVisitPayloadWithoutRelations(visitRecord);
    const derivedRefresh = createDerivedRefreshTargets();
    const canSaveLightweight = typeof this.remote.saveVisitRecord === "function";

    if (canSaveLightweight) {
      await this.remote.saveVisitRecord(this.defaultClinicId, visitRecord.animalId, visitPayload);
      addDerivedRefreshForVisit(derivedRefresh, visitRecord, settings.previousRecord);
    } else {
  ...
  SyncedEmbryoRepository.prototype.applyMergedSnapshotToRemote = async function (
    ...
    for (let index = 0; index < mergedVisits.length; index += 1) {
      ...
      if (!remoteVisit || hasDifferentRecord(remoteVisit, visitRecord)) {
        const result = await this.upsertRemoteVisit(visitRecord, {
          previousRecord: remoteVisit,
        });
    ...
    if (hasDerivedRefreshTargets(derivedRefresh)) {
      this.addPendingDerivedRefresh(derivedRefresh);
      await this.flushPendingDerivedRefresh();
      remoteChanged = true;
    }
