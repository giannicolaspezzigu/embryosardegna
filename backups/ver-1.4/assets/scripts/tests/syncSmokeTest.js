(function () {
  ...
  async function testLocalWriteDuringSyncIsNotLost() {
    ...
    try {
      ...
      assertEqual(repository.getSyncState().pendingCount, 1, "La visita scritta durante lo sync non e' rimasta pendente");

      await repository.syncNow();

      const remoteVisits = await remote.listAnimalVisits(TEST_CLINIC_ID, animal.id);
      assertEqual(remoteVisits.length, 1, "La visita scritta durante lo sync non e' arrivata sul remoto al giro successivo");
