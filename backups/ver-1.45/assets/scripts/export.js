(function () {
  ...
  const PDF_FALLBACK_HOURS = [10, 2, 4, 8, 12, 6, 1, 5, 7, 11, 3, 9];
  const PDF_SIDE_HOURS = [12, 3, 6, 9];
  const PDF_LAYOUT = {
    margin: 10,
    reportHeaderHeight: 15,
    animalHeaderHeight: 24,
    cardGapX: 4,
    cardGapY: 5,
    visitCardHeight: 66,
    footerHeight: 7,
  ...
  function buildVisitExportRecord(animal, visit) {
    ...
    return {
      ...
      visitPurpose: visit && visit.visitPurpose ? visit.visitPurpose : "",
      operatorName: visit && visit.operatorName ? visit.operatorName : "",
      programType: visit && visit.protocolContext && visit.protocolContext.programType ? visit.protocolContext.programType : "",
      protocolName: visit && visit.protocolContext && visit.protocolContext.protocolName ? visit.protocolContext.protocolName : "",
  ...
  function buildSuggestedVisitImagesFileName(filters) {
    const settings = filters || {};
    ...
    if (settings.sessionId && settings.sessionId !== "all") {
      ...
      if (sessionName) {
        return ensurePdfExtension(`${sessionName}_immagini_visite`);
      }
    }

    return `embryosardegna_immagini_visite_${buildExportDateStamp()}.pdf`;
  }
  ...
  async function savePdfBuffer(buffer, fileName) {
    const normalizedFileName = ensurePdfExtension(sanitizeFileNameSegment(fileName));
    ...
    downloadPdfBuffer(buffer, normalizedFileName);
    return true;
  }
  ...
  function formatPdfDateTime(value) {
    if (app.utils && typeof app.utils.formatShortDateTime === "function") {
      return app.utils.formatShortDateTime(value);
    }

    return formatPdfDateOnly(value);
  }

  function humanizePdfEnum(mapName, value, fallbackValue) {
    if (app.utils && typeof app.utils.humanizeEnum === "function" && app.config && app.config[mapName]) {
      return app.utils.humanizeEnum(app.config[mapName], value, fallbackValue);
    }
  ...
  function truncatePdfText(doc, value, maxWidth) {
    const rawText = textOrDash(value);

    if (doc.getTextWidth(rawText) <= maxWidth) {
      return rawText;
    }
  ...
  function getPdfPageWidth(doc) {
    return typeof doc.internal.pageSize.getWidth === "function" ? doc.internal.pageSize.getWidth() : doc.internal.pageSize.width;
  }

  function getPdfPageHeight(doc) {
    return typeof doc.internal.pageSize.getHeight === "function" ? doc.internal.pageSize.getHeight() : doc.internal.pageSize.height;
  }
  ...
  function setPdfDrawColor(doc, color) {
    doc.setDrawColor(color[0], color[1], color[2]);
  }

  function setPdfFillColor(doc, color) {
    doc.setFillColor(color[0], color[1], color[2]);
  }

  function normalizePdfStructureType(type) {
  ...
  function summarizePdfPreviewStructures(structures) {
    const summary = {
      totalFollicles: 0,
      smallFollicles: 0,
      mediumFollicles: 0,
    ...
    (structures || []).forEach((structure) => {
      const type = normalizePdfStructureType(structure.type);

      if (type === "follicle") {
        const size = toFiniteNumber(structure.sizeMm) || toFiniteNumber(structure.size);
  ...
  function representativePdfFollicleSize(category, counts, index) {
    if (category === "small") {
    ...
    const largest = toFiniteNumber(counts.largestFollicleMm);
    return largest !== null ? Math.max(5.5, largest - index * 0.5) : 6;
  }

  function pushSyntheticPdfStructures(structures, side, type, count, factory) {
        ...
        Object.assign(
          {
            side,
            type,
            clockHour: PDF_FALLBACK_HOURS[nextIndex % PDF_FALLBACK_HOURS.length],
  ...
  function buildPdfStructuresForSide(visit, sideKey, side) {
    const sideRecord = visit && visit.ovaries && visit.ovaries[sideKey] ? visit.ovaries[sideKey] : {};
    ...
    const structures = Array.isArray(sideRecord.structures)
      ...
      : [];
    const initial = summarizePdfPreviewStructures(structures);

    pushSyntheticPdfStructures(
      structures,
      side,
      "follicle",
      Math.max(0, (toFiniteNumber(counts.smallFollicles) || 0) - initial.smallFollicles),
      (index) => ({ sizeMm: representativePdfFollicleSize("small", counts, index) })
    );
    ...
    pushSyntheticPdfStructures(
      ...
      side,
      "follicle",
      Math.max(0, (toFiniteNumber(counts.mediumFollicles) || 0) - initial.mediumFollicles),
      (index) => ({ sizeMm: representativePdfFollicleSize("medium", counts, index) })
    );
    ...
    pushSyntheticPdfStructures(
      ...
      side,
      "follicle",
      Math.max(0, (toFiniteNumber(counts.largeFollicles) || 0) - initial.largeFollicles),
      (index) => ({ sizeMm: representativePdfFollicleSize("large", counts, index) })
    );

    const afterFollicleBuckets = summarizePdfPreviewStructures(structures);
    pushSyntheticPdfStructures(
      structures,
      side,
      "follicle",
      Math.max(0, (toFiniteNumber(counts.totalFollicles) || 0) - afterFollicleBuckets.totalFollicles),
      () => ({ sizeMm: 4 })
    );

    const afterFollicles = summarizePdfPreviewStructures(structures);
    pushSyntheticPdfStructures(structures, side, "ovulation", Math.max(0, (toFiniteNumber(counts.ovulations) || 0) - afterFollicles.ovulations));
    pushSyntheticPdfStructures(
      structures,
      side,
      "corpus_luteum",
      Math.max(0, (toFiniteNumber(counts.corporaLutea) || 0) - afterFollicles.corporaLutea)
    );
    pushSyntheticPdfStructures(
      structures,
      side,
      "cyst",
      Math.max(0, (toFiniteNumber(counts.cysts) || 0) - afterFollicles.cysts),
      () => ({ sizeMm: 8 })
    );
  ...
  function getPdfClockPosition(clockHour, index, rect) {
    const hour = toFiniteNumber(clockHour) || PDF_FALLBACK_HOURS[index % PDF_FALLBACK_HOURS.length];
    const angle = (hour / 12) * Math.PI * 2 - Math.PI / 2;

    return {
      x: rect.centerX + rect.radiusX * 0.58 * Math.cos(angle),
      y: rect.centerY + rect.radiusY * 0.58 * Math.sin(angle),
    };
  }

  function getPdfStructurePosition(structure, index, rect) {
    const position = structure.position || {};
    const positionX = toFiniteNumber(position.x);
    const positionY = toFiniteNumber(position.y);

    if (position.normalized && positionX !== null && positionY !== null) {
      return {
        x: rect.x + clampPdfNumber(positionX * rect.width, rect.width * 0.12, rect.width * 0.88),
        y: rect.y + clampPdfNumber(positionY * rect.height, rect.height * 0.2, rect.height * 0.88),
      };
    }

    const rawX = positionX !== null ? positionX : toFiniteNumber(structure.x);
    const rawY = positionY !== null ? positionY : toFiniteNumber(structure.y);

    if (rawX !== null && rawY !== null) {
      const scaledX = rawX <= 1 ? rawX * rect.width : (rawX / 280) * rect.width;
      const scaledY = rawY <= 1 ? rawY * rect.height : (rawY / 220) * rect.height;

      return {
        x: rect.x + clampPdfNumber(scaledX, rect.width * 0.12, rect.width * 0.88),
        y: rect.y + clampPdfNumber(scaledY, rect.height * 0.2, rect.height * 0.88),
      };
    }

    return getPdfClockPosition(structure.clockHour, index, rect);
  }

  function drawPdfStructure(doc, structure, side, index, rect) {
    const type = normalizePdfStructureType(structure.type);
    const position = getPdfStructurePosition(structure, index, rect);
    const sideColor = side === "L" ? [0, 154, 117] : [38, 128, 202];

    if (type === "follicle") {
      const size = toFiniteNumber(structure.sizeMm) || toFiniteNumber(structure.size) || 4;
      const radius = clampPdfNumber(size * 0.38, 1.45, 4);
      const fontSize = clampPdfNumber(radius * 1.25, 3.2, 4.7);

      setPdfFillColor(doc, side === "L" ? [230, 250, 244] : [232, 245, 255]);
      setPdfDrawColor(doc, sideColor);
      doc.setLineWidth(0.25);
      doc.circle(position.x, position.y, radius, "FD");
      doc.setFont("helvetica", "bold");
      doc.setFontSize(fontSize);
      setPdfTextColor(doc, sideColor);
      doc.text(textOrDash(size), position.x, position.y + fontSize * 0.11, { align: "center", baseline: "middle" });
      return;
    }

    if (type === "cyst") {
      const size = toFiniteNumber(structure.sizeMm) || toFiniteNumber(structure.size) || 8;
      const radius = clampPdfNumber(size * 0.3, 2.2, 4.3);

      setPdfFillColor(doc, [255, 246, 237]);
      doc.setDrawColor(255, 144, 66);
      ...
      doc.text("CY", position.x, position.y, { align: "center", baseline: "middle" });
      return;
    }
    ...
    if (type === "ovulation") {
      ...
      doc.circle(position.x, position.y, radius + 0.7, "S");
      doc.line(position.x - radius, position.y, position.x + radius, position.y);
      doc.line(position.x, position.y - radius, position.x, position.y + radius);
      doc.line(position.x - radius * 0.7, position.y - radius * 0.7, position.x + radius * 0.7, position.y + radius * 0.7);
      doc.line(position.x + radius * 0.7, position.y - radius * 0.7, position.x - radius * 0.7, position.y + radius * 0.7);
      return;
    }

    if (type === "corpus_luteum") {
      setPdfFillColor(doc, [255, 250, 225]);
      doc.setDrawColor(202, 150, 20);
      doc.setLineWidth(0.35);
      doc.circle(position.x, position.y, 3.4, "FD");
      doc.setFont("helvetica", "bold");
      doc.setFontSize(3.7);
      doc.setTextColor(150, 110, 12);
      doc.text("CL", position.x, position.y, { align: "center", baseline: "middle" });
    }
  }

  function drawPdfOvarySide(doc, visit, sideKey, side, label, x, y, width, height) {
    const sideColor = side === "L" ? [0, 154, 117] : [38, 128, 202];
    const rect = {
      x,
      y,
    ...
    const structures = buildPdfStructuresForSide(visit, sideKey, side);

    doc.setFont("helvetica", "bold");
    doc.setFontSize(5.2);
    setPdfTextColor(doc, sideColor);
    doc.text(label, x + 1.8, y + 4.6);

    doc.setFont("helvetica", "normal");
    doc.setFontSize(3.6);
    PDF_SIDE_HOURS.forEach((hour) => {
      const angle = (hour / 12) * Math.PI * 2 - Math.PI / 2;
      const hourX = rect.centerX + rect.radiusX * 1.28 * Math.cos(angle);
      const hourY = rect.centerY + rect.radiusY * 1.25 * Math.sin(angle);
      doc.text(String(hour), hourX, hourY, { align: "center", baseline: "middle" });
    });

    setPdfFillColor(doc, side === "L" ? [238, 252, 247] : [238, 247, 255]);
    setPdfDrawColor(doc, sideColor);
    doc.setLineWidth(0.4);
    doc.ellipse(rect.centerX, rect.centerY, rect.radiusX, rect.radiusY, "FD");

    structures.forEach((structure, index) => {
      drawPdfStructure(doc, structure, side, index, rect);
    });
  }

  function buildPdfFilterSummary(filters) {
    const settings = filters || {};
    const sessions = app.state.workspace.sessions || [];
    const session = sessions.find((candidate) => candidate.id === settings.sessionId);
    const parts = [];

    parts.push(settings.sessionId && settings.sessionId !== "all" ? `Sessione: ${session ? session.name : settings.sessionId}` : "Tutte le sessioni");

    if (settings.farmName && settings.farmName !== "all") {
      parts.push(`Allevamento: ${settings.farmName}`);
    }

    if (settings.dateFrom || settings.dateTo) {
      parts.push(`Date: ${settings.dateFrom || "..."} - ${settings.dateTo || "..."}`);
    }

    if (settings.operatorName) {
      parts.push(`Operatore: ${settings.operatorName}`);
    }

    return parts.join(" | ");
  }

  function drawPdfReportHeader(doc, filters) {
    ...
    doc.setFont("helvetica", "bold");
    doc.setFontSize(14);
    doc.setTextColor(28, 44, 35);
    doc.text("Export immagini visite", margin, y + 2.5);

    doc.setFont("helvetica", "normal");
    doc.setFontSize(7.2);
    doc.setTextColor(82, 96, 87);
    doc.text(truncatePdfText(doc, buildPdfFilterSummary(filters), pageWidth - margin * 2), margin, y + 9.5);

    doc.setDrawColor(204, 214, 207);
    doc.setLineWidth(0.25);
    doc.line(margin, y + PDF_LAYOUT.reportHeaderHeight - 1, pageWidth - margin, y + PDF_LAYOUT.reportHeaderHeight - 1);

    return y + PDF_LAYOUT.reportHeaderHeight;
  }

  function drawPdfAnimalHeader(doc, group, animalIndex, y, continued) {
    const animal = group.animal || {};
    const margin = PDF_LAYOUT.margin;
    const pageWidth = getPdfPageWidth(doc);
    const width = pageWidth - margin * 2;
    const height = PDF_LAYOUT.animalHeaderHeight;
    const title = `${animalIndex}. ${getDisplayAnimalId(animal) || "Animale"}${continued ? " (continua)" : ""}`;
    const metaLine1 =
      `Allevamento: ${textOrDash(animal.farmName)} | Sessione: ${textOrDash(animal.sessionName)} | ` +
      `Specie: ${humanizePdfEnum("speciesLabels", animal.species, "Specie")} | Razza: ${textOrDash(animal.breed)}`;
    const metaLine2 =
      `Nascita: ${formatPdfDateOnly(animal.birthDate)} | Ultimo parto: ${formatPdfDateOnly(animal.lastParturitionDate)} | ` +
      `BCS: ${textOrDash(animal.bodyConditionScore)} | Parita: ${textOrDash(animal.parity)} | Visite esportate: ${group.visits.length}`;

    setPdfFillColor(doc, [247, 250, 247]);
    doc.setDrawColor(204, 214, 207);
    doc.setLineWidth(0.25);
    ...
    doc.setTextColor(28, 44, 35);
    doc.text(truncatePdfText(doc, title, width - 8), margin + 4, y + 7);

    doc.setFont("helvetica", "normal");
    doc.setFontSize(6.6);
    doc.setTextColor(72, 86, 77);
    doc.text(truncatePdfText(doc, metaLine1, width - 8), margin + 4, y + 14);
    doc.text(truncatePdfText(doc, metaLine2, width - 8), margin + 4, y + 20);

    return y + height + 4;
  }

  function drawPdfVisitCard(doc, record, x, y, width, height) {
    const visit = record.rawVisit || {};
    const purpose = humanizePdfEnum("visitPurposeLabels", record.visitPurpose || visit.visitPurpose, "Ecografia");
    const program = humanizePdfEnum("programTypeLabels", record.programType || (visit.protocolContext && visit.protocolContext.programType), "Programma n.d.");
    const protocolName = record.protocolName || (visit.protocolContext && visit.protocolContext.protocolName) || "";
    const operator = record.operatorName || visit.operatorName || "-";
    const corpusLuteumCount = (record.corpusLuteumAreas || []).length;
    const visitBcs = visit.bodyConditionScore !== null && visit.bodyConditionScore !== undefined ? visit.bodyConditionScore : record.animalBodyConditionScore;
    const title = formatPdfDateTime(visit.visitAt);
    const metaLine = `${purpose} | ${program}${protocolName ? ` | ${protocolName}` : ""}`;
    const totalsLine =
      `F tot ${record.totalFollicles || 0} | piccoli ${record.smallFollicles || 0} | medi ${record.mediumFollicles || 0} | grandi ${record.largeFollicles || 0}`;
    const clinicalLine = `CL ${corpusLuteumCount || 0} | OV ${record.ovulationCount || 0} | DPP ${textOrDash(record.postPartumDays)} | BCS ${textOrDash(visitBcs)}`;
    const summaryLine = `SX ${record.leftSummary || "/"} | DX ${record.rightSummary || "/"}`;
    const ovaryY = y + 18;
    const ovaryWidth = (width - 12) / 2;
    const ovaryHeight = 32;

    setPdfFillColor(doc, [252, 253, 251]);
    doc.setDrawColor(204, 214, 207);
    doc.setLineWidth(0.25);
    doc.roundedRect(x, y, width, height, 2, 2, "FD");

    setPdfFillColor(doc, [236, 246, 241]);
    doc.rect(x, y, width, 13, "F");

    doc.setFont("helvetica", "bold");
    doc.setFontSize(7.3);
    doc.setTextColor(28, 44, 35);
    doc.text(truncatePdfText(doc, title, width - 6), x + 3, y + 5.2);

    doc.setFont("helvetica", "normal");
    doc.setFontSize(5.3);
    doc.setTextColor(80, 92, 84);
    doc.text(truncatePdfText(doc, `Operatore: ${operator}`, width - 6), x + 3, y + 10.3);
    doc.text(truncatePdfText(doc, metaLine, width - 6), x + 3, y + 16.1);

    drawPdfOvarySide(doc, visit, "left", "L", "SX", x + 3, ovaryY, ovaryWidth, ovaryHeight);
    drawPdfOvarySide(doc, visit, "right", "R", "DX", x + 8 + ovaryWidth, ovaryY, ovaryWidth, ovaryHeight);

    doc.setFont("helvetica", "normal");
    doc.setFontSize(5.1);
    doc.setTextColor(62, 76, 68);
    doc.text(truncatePdfText(doc, totalsLine, width - 6), x + 3, y + height - 11.3);
    doc.text(truncatePdfText(doc, clinicalLine, width - 6), x + 3, y + height - 6.8);
    doc.text(truncatePdfText(doc, summaryLine, width - 6), x + 3, y + height - 2.5);
  }
  ...
  function addPdfFooters(doc) {
    ...
    const margin = PDF_LAYOUT.margin;
    const pageWidth = getPdfPageWidth(doc);
    const pageHeight = getPdfPageHeight(doc);
    ...
    for (let pageNumber = 1; pageNumber <= pageCount; pageNumber += 1) {
      ...
      doc.text(`Embryosardegna | pagina ${pageNumber}/${pageCount}`, pageWidth - margin, pageHeight - 4.5, {
        align: "right",
      });
    }
  }

  function buildVisitImagesPdf(groups, filters) {
    ...
    const pageWidth = getPdfPageWidth(doc);
    const pageHeight = getPdfPageHeight(doc);
    const contentBottom = pageHeight - margin - PDF_LAYOUT.footerHeight;
    const cardWidth = (pageWidth - margin * 2 - PDF_LAYOUT.cardGapX * (PDF_LAYOUT.cardsPerRow - 1)) / PDF_LAYOUT.cardsPerRow;
    let cursorY = drawPdfReportHeader(doc, filters);

    if (!groups.length) {
      doc.setFont("helvetica", "bold");
      doc.setFontSize(11);
      doc.setTextColor(50, 64, 55);
      doc.text("Nessuna visita trovata con i filtri selezionati.", margin, cursorY + 12);
      addPdfFooters(doc);
      return doc;
    }
    ...
    for (let groupIndex = 0; groupIndex < groups.length; groupIndex += 1) {
      ...
      if (cursorY + PDF_LAYOUT.animalHeaderHeight + PDF_LAYOUT.visitCardHeight > contentBottom) {
        doc.addPage();
        cursorY = drawPdfReportHeader(doc, filters);
      }

      cursorY = drawPdfAnimalHeader(doc, group, groupIndex + 1, cursorY, false);

      for (let visitIndex = 0; visitIndex < group.visits.length; visitIndex += PDF_LAYOUT.cardsPerRow) {
        if (cursorY + PDF_LAYOUT.visitCardHeight > contentBottom) {
          doc.addPage();
          cursorY = drawPdfReportHeader(doc, filters);
          cursorY = drawPdfAnimalHeader(doc, group, groupIndex + 1, cursorY, true);
        }

        const rowVisits = group.visits.slice(visitIndex, visitIndex + PDF_LAYOUT.cardsPerRow);

        rowVisits.forEach((record, columnIndex) => {
          const x = margin + columnIndex * (cardWidth + PDF_LAYOUT.cardGapX);
          drawPdfVisitCard(doc, record, x, cursorY, cardWidth, PDF_LAYOUT.visitCardHeight);
        });

        cursorY += PDF_LAYOUT.visitCardHeight + PDF_LAYOUT.cardGapY;
      }
  ...
  app.exporter = {
    init() {
      ...
      if (app.dom.refs.exportVisitImagesBtn) {
        app.dom.refs.exportVisitImagesBtn.addEventListener("click", () => {
          this.exportVisitImagesPdf().catch((error) => {
            console.error(error);
            app.ui.toast("Errore durante l'export immagini visite", "warn");
          });
        });
      }
    },
    ...
    async exportVisitImagesPdf() {
      if (!assertJsPdfAvailable()) {
        app.ui.toast("Libreria PDF non disponibile nella pagina", "warn");
        return;
      }

      const filters = buildExportFiltersFromDom();
      const fileName = buildSuggestedVisitImagesFileName(filters);
      const groups = await collectExportGroups(filters);
      const pdf = buildVisitImagesPdf(groups, filters);
      const buffer = pdf.output("arraybuffer");
      const saved = await savePdfBuffer(buffer, fileName);
      ...
      app.ui.toast(groups.length ? "PDF immagini visite esportato" : "PDF esportato: nessuna visita trovata");
    },
  };
})();
