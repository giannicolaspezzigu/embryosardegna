(function () {
  ...
  FirestoreEmbryoRepository.prototype.saveVisitRecord = async function (clinicId, animalId, visitPayload) {
    const normalizedVisit = app.domain.normalizers.visit(
      app.domain.helpers.mergeDeep(visitPayload || {}, {
        clinicId,
        animalId,
  ...
  FirestoreEmbryoRepository.prototype.getSyncTombstone = async function (clinicId, tombstoneKey) {
    const resolvedClinicId = clinicId || this.defaultClinicId;
    ...
    const snapshot = await this.syncTombstonesCollection(resolvedClinicId).doc(tombstoneKey).get();
    return snapshot.exists ? cloneTombstone(snapshot.data()) : null;
  };
