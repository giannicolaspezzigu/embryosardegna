(function () {
  ...
  function getSaveVisitButtonLabel() {
    if (editorState.saving) {
      return editorState.mode === "edit" && editorState.editingVisitId ? "Aggiornamento..." : "Salvataggio...";
    }

    if (editorState.mode === "edit" && editorState.editingVisitId) {
      return editorState.dirty ? "Aggiorna visita" : "Visita salvata";
    }

    return "Salva visita";
  }

  function updateSaveVisitButtonLabel() {
    if (app.dom && app.dom.refs && app.dom.refs.saveVisitBtn) {
      app.dom.refs.saveVisitBtn.textContent = getSaveVisitButtonLabel();
      app.dom.refs.saveVisitBtn.disabled =
        editorState.saving || (editorState.mode === "edit" && editorState.editingVisitId && !editorState.dirty);
    }
  }

  function setEditorDirty(dirty) {
    editorState.dirty = Boolean(dirty);
    updateSaveVisitButtonLabel();
  }
  ...
  function getCurrentEditingVisitForMode() {
    const selectedVisit = app.state.workspace.selectedVisit;

    if (selectedVisit && selectedVisit.id === editorState.editingVisitId) {
      return selectedVisit;
    }

    return {
      id: editorState.editingVisitId,
      visitAt: app.utils.localDateTimeInputToIso(app.dom.refs.visitDatetimeInput.value),
    };
  }
  ...
  function calculatePostPartumDays(lastParturitionDate, visitAt) {
    const parturition = getLocalDateParts(lastParturitionDate, true);
    const visit = getLocalDateParts(visitAt, false);

    if (!parturition || !visit) {
      return "";
    }

    const diffMs = toUtcDateOnly(visit).getTime() - toUtcDateOnly(parturition).getTime();
  ...
  function updatePostPartumDaysDisplay() {
    if (!app.dom || !app.dom.refs || !app.dom.refs.postPartumDaysInput) {
      return;
    }

    const selectedAnimal = app.state.workspace.selectedAnimal;
    const postPartumDays = calculatePostPartumDays(
      selectedAnimal && selectedAnimal.lastParturitionDate,
      app.dom.refs.visitDatetimeInput.value
    );

    app.dom.refs.postPartumDaysInput.value =
      postPartumDays !== "" ? `${postPartumDays} ${postPartumDays === 1 ? "giorno" : "giorni"}` : "N/D";
  }

  function getPreviousBodyConditionScore() {
    const selectedAnimal = app.state.workspace.selectedAnimal;
    const animalId = app.state.context.activeAnimalId || (selectedAnimal ? selectedAnimal.id : null);
    const visits = animalId ? app.state.workspace.visitsByAnimalId[animalId] || [] : [];
    ...
    if (selectedAnimal && selectedAnimal.bodyConditionScore !== null && selectedAnimal.bodyConditionScore !== undefined) {
      return selectedAnimal.bodyConditionScore;
    }
  ...
  function getDefaultContextValues() {
    return {
      ...
      bodyConditionScore: getPreviousBodyConditionScore(),
  ...
  app.visitEditor = {
    ...
    bindEvents() {
      ...
      const dirtySelector =
        "input, select, textarea, button.uparam-btn, button.draft-remove-btn, #pregnancyCheckEnabledInput";
      const editorRoot = document.querySelector(".editor-focus-section");

      if (editorRoot) {
        editorRoot.addEventListener("input", (event) => {
          if (event.target.closest(dirtySelector)) {
            markEditorDirty();
          }
        });
        ...
        editorRoot.addEventListener("click", (event) => {
          if (event.target.closest("button.uparam-btn")) {
            markEditorDirty();
          }
        });
      }
      ...
      app.dom.refs.resetVisitEditorBtn.addEventListener("click", () => {
        const preserveCurrentEdit = editorState.mode === "edit" && editorState.editingVisitId;
        this.resetEditor({ preserveCurrentEdit });
        app.ui.toast(preserveCurrentEdit ? "Editor ripristinato: puoi aggiornare la visita corrente" : "Editor visita ripristinato");
      });
    ...
    resetEditor(options) {
      ...
      const preserveCurrentEdit = Boolean(settings.preserveCurrentEdit && editorState.editingVisitId);
      const editingVisitForMode = preserveCurrentEdit ? getCurrentEditingVisitForMode() : null;
      ...
      renderAttachmentDrafts();
      renderProtocolEventDrafts();
      setEditorMode(preserveCurrentEdit ? "edit" : "new", editingVisitForMode);
      setEditorDirty(preserveCurrentEdit);
    },
    ...
    async saveCurrentVisit() {
      ...
      if (isEditing && !editorState.dirty) {
        app.ui.toast("La visita corrente e gia salvata");
        updateSaveVisitButtonLabel();
        return;
      }
      ...
      if (!isEditing && !hasMeaningfulVisitData(payload, attachments, protocolEvents, pregnancyCheck)) {
      ...
      try {
        ...
        setEditorMode(
          "edit",
          app.state.workspace.selectedVisit && app.state.workspace.selectedVisit.id === savedVisit.id
            ? app.state.workspace.selectedVisit
            : savedVisit
        );
        setEditorDirty(false);
